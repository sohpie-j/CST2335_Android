    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `ToDoItem` (`id` INTEGER PRIMARY KEY AUTOINCREMENT, `content` TEXT NOT NULL)');
  ...
  Future<List<ToDoItem>> findAllToDoItems() async {
    return _queryAdapter.queryList('SELECT * FROM ToDoItem',
        mapper: (Map<String, Object?> row) =>
            ToDoItem(id: row['id'] as int?, content: row['content'] as String));
  }

  @override
  Future<void> deleteToDoItemByContent(String content) async {
    await _queryAdapter.queryNoReturn('DELETE FROM ToDoItem WHERE content = ?1',
        arguments: [content]);
  }
